...
from hardware_emulated.conv import *
from hardware_emulated.conv_fp32 import *
from hardware_emulated.fc_layer import *
from hardware_emulated.input_generator import *
from hardware_emulated.torch_net import *
from hardware_emulated.weight_loader import *
...
class omniglot_hd_emulation:
	...
	def Network(self, img, kernel_size, stride, dict):
		conv1 = conv_3_3(img, kernel_size, stride, self.params['no_filters'], dict['cv1.bias'],
		                 self.params['activation'], self.params['fractional'], self.params['decimal'])
		conv2 = conv_3_3(conv1.forward(dict['cv1.weight']), kernel_size, stride, self.params['no_filters'],
		                 dict['cv2.bias'], self.params['activation'], self.params['fractional'], self.params['decimal'])
		conv3 = conv_3_3(conv2.forward(dict['cv2.weight']), kernel_size, stride, self.params['no_filters'],
		                 dict['cv3.bias'], self.params['activation'], self.params['fractional'], self.params['decimal'])
		conv4 = conv_3_3(conv3.forward(dict['cv3.weight']), kernel_size, stride, self.params['no_filters'],
		                 dict['cv4.bias'], self.params['activation'], self.params['fractional'], self.params['decimal'])
	...
	def Network_fp(self, img, kernel_size, stride, dict):
		...
		conv2 = conv_3_3_fp(conv1.forward(dict['cv1.weight']), kernel_size, stride, self.params['no_filters'],
		                    dict['cv2.bias'], self.params['activation'])
		conv3 = conv_3_3_fp(conv2.forward(dict['cv2.weight']), kernel_size, stride, self.params['no_filters'],
		                    dict['cv3.bias'], self.params['activation'])
		conv4 = conv_3_3_fp(conv3.forward(dict['cv3.weight']), kernel_size, stride, self.params['no_filters'],
		                    dict['cv4.bias'], self.params['activation'])
		# print (conv4.forward(dict['cv4.weight']), "This is the final conv layer output")
		return conv4.forward(dict['cv4.weight'])

	def plastic_layer(self, input_activations, label, dict, mod):
		fully_connected = FC_layer(input_activations, label, dict['eta'], self.params['fractional'],
		                           self.params['decimal'])
...
def train(parameters):
	...
	for num_test_sample in range(params['no_test_iters']):
		...
		for i in range(inputs.shape[0]):
			...
			torch_output_vector, torch_final_out, mod_torch = net(Variable(input_activations, requires_grad=False),
			                                                      Variable(label, requires_grad=False), mod_torch)
