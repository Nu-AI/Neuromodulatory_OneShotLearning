...
for num_iter in range(params['num_iters']):
	...
	inputs, labels, targets = input_retriever.gen_train_test_split(input_dataset, test=0)
	trace = OSL_net.initalize_trace()
	for _ in range(params['nb_steps']):
		feature_activ, activout, trace = OSL_net.forward(inputs['step'], labels['step'], trace)

	loss = criterion(activout[0], targets)
	curr_loss = loss.data
	...
	if not params['test']:
		...
		print(num_iter, "====")
		td = targets.cpu().numpy()
		yd = activout.data.cpu().numpy()[0]
		print("y: ", yd[:10])
		print("target: ", td[:10])
