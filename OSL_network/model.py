...
class Network(nn.Module):
	def __init__(self, params):
		...
		self.w = torch.nn.Parameter((.01 * torch.randn(params['no_filters'], params['no_classes'])).cuda(),
		                            requires_grad=True)

		# self.w =  torch.nn.Parameter((.01 * torch.rand(params['plastsize'], params['no_classes'])).cuda(), requires_grad=True)
		# if params['alpha'] == 'free':
		self.alpha = torch.nn.Parameter((.01 * torch.rand(params['no_filters'], params['no_classes'])).cuda(),
		                                requires_grad=True)  # Note: rand rather than randn (all positive)
	...
	def forward(self, inputx, inputlabel, hebb):
		...
		activ = activin.mm(self.w + torch.mul(self.alpha,
		                                      hebb)) + 1000.0 * inputlabel  # The expectation is that a nonzero inputlabel will overwhelm the inputs and clamp the outputs
		...
		print("\n", activ,
		      "These are the intermediate activations before softmax function in the torchmodel \n %%%%%%%%%%%%%%%%%%")
		activout = F.softmax(activ)
		print("\n", activout,
		      "These are the intermediate activations after softmax function in the torchmodel \n %%%%%%%%%%%%%%%%%%")
		# if self.rule == 'hebb':
		...
		hebb = hebb + self.eta * torch.mul((activin[0].unsqueeze(1) - torch.mul(hebb, activout[0].unsqueeze(0))),
		                                   activout[0].unsqueeze(
			                                   0))  # Oja's rule. Remember that yin, yout are row vectors (dim (1,N)). Also, broadcasting!
